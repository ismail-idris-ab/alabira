...
<tr><td style="background:#2E6B3E;padding:32px 40px;border-radius:12px 12px 0 0;">
  <h1 style="margin:0;color:#fff;font-size:22px;font-family:Georgia,serif;">Alabira Global Farm Tilde</h1>
...
<tr><td style="background:#4A3B32;padding:20px 40px;border-radius:0 0 12px 12px;text-align:center;">
  <p style="margin:0;color:rgba(255,255,255,0.6);font-size:12px;">© 2025 Alabira Global Farm Tilde ·
...
const sendContactNotification = (contact) => {
  ...
  return send(
    ...
    wrap('New Contact Submission', `
      ...
      <blockquote style="border-left:4px solid #2E6B3E;margin:0;padding:12px 20px;background:rgba(46,107,62,0.06);">${contact.message}</blockquote>
      <p style="margin-top:24px;font-size:12px;color:#999;">Submitted: ${date} · IP: ${contact.ipAddress || 'N/A'}</p>
    `)
  );
};

const sendContactConfirmation = (email, name) =>
  send(email, "We've received your message — Alabira Global Farm Tilde",
    wrap('Message Received', `
      <p>Dear ${name},</p>
      <p>Thank you for contacting Alabira Global Farm Tilde. We will reply within <strong>24 hours</strong>.</p>
...
const sendUnsubscribeConfirmation = (email) =>
  send(email, "You've been unsubscribed — Alabira Global Farm Tilde",
    wrap('Unsubscribed', `
